...
const UserModel = {
  ...
  effects: {
    ...
    *addUserWallet({ payload }, { call, put }) {
      ...
      if (response.status === 'success') {
        message.success(formatMessage({id:'models.user.add_wallet_address_success'}));
    ...
    *updateUserWallet({ payload }, { call, put }) {
      ...
      if (response.status === 'success') {
        message.success(formatMessage({id:'models.user.edit_wallet_address_success'}));
        window.location.reload();
      }
    },
    ...
    *postUserAvatar({ payload }, { call, put }) {
      ...
      if (response.status === 'success') {
        message.success(formatMessage({id:'models.user.userportrait_update_success'}));
        window.location.reload();
        // yield put({
    ...
    *postUserInfo({ payload }, { call, put }) {
      ...
      if (response.status === 'success') {
        message.success(formatMessage({id:'models.user.personal_information_update_success'}));
    ...
    *postForgetPwd({ payload }, { call, put }) {
      ...
      if (response.status === 'success') {
        message.success(formatMessage({id:'models.user.send_email'},{payload_email:payload.email}));
        return true;
      }

      if (response.status === 'fail') {
        if (response.code === 404) {
          message.error(formatMessage({id:'models.user.email_nonexist'},{payload_email:payload.email}));
        }
      }
    ...
    *postResetPwd({ payload }, { call, put }) {
      ...
      if (response.status === 'success') {
        message.success(formatMessage({id:'models.user.password_reset_success'}));
      ...
      if (response.status === 'fail') {
        if (response.message === 'SignatureExpired') {
          message.error(formatMessage({id:'models.user.reset_link_overdue'}));
        } else if (response.message === 'DoubleCheckError') {
          message.error(formatMessage({id:'models.user.email_error'},{payload_email:payload.email}));
        } else {
          message.error(formatMessage({id:'models.user.password_reset_unsuccess'}));
        }
      }
    },
  },
